...
" g++/gcc.
autocmd BufEnter *.cpp,*.c,*.h,*.hpp set errorformat=%f:%l:%c:\ %m,%-G%.%#

" Go compiler.
autocmd BufEnter *.go set errorformat=%\./%f:%l:%c:\ %m,%-G%.%#

" Python. Stolen from :h errorformat.
autocmd BufEnter *.py set errorformat=%C\ %.%#,%A\ \ File\ \"%f\"\\,\ line\ %l%.%#,%Z%[%^\ ]%\\@=%m,%-G%.%#
